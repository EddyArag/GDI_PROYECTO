...
public class CargaDb4 {
    public static void ejecutar(Connection conn) {
        try (Statement st = conn.createStatement()) {
            ...
            // st.executeUpdate("DROP TRIGGER IF EXISTS TR_NORMALIZAR_STOCK ON
            // Servicio_Producto;");
            // st.executeUpdate("CREATE TRIGGER TR_NORMALIZAR_STOCK BEFORE INSERT OR UPDATE
            // ON Servicio_Producto FOR EACH ROW EXECUTE FUNCTION F_NORMALIZAR_STOCK();");

            // FUNCIÓN DEL TRIGGER: F_VALIDAR_RUC_DNI
            st.executeUpdate(
                    "CREATE OR REPLACE FUNCTION F_VALIDAR_RUC_DNI() " +
                            "RETURNS TRIGGER AS $$ " +
                            "DECLARE v_ruc_text TEXT; " +
                            "BEGIN " +
                            "    v_ruc_text := TRIM(BOTH FROM COALESCE(NEW.RUC::text, '')); " +
                            "    IF v_ruc_text <> '' THEN " +
                            "        IF v_ruc_text !~ '^[0-9]+$' THEN " +
                            "            RAISE EXCEPTION 'ERROR DE VALIDACIÓN (RUC/DNI): El campo RUC/DNI debe contener solo números.'; "
                            +
                            "        END IF; " +
                            "        IF CHAR_LENGTH(v_ruc_text) NOT IN (8, 11) THEN " +
                            "            RAISE EXCEPTION 'ERROR DE VALIDACIÓN (RUC/DNI): El número de RUC/DNI debe ser de 8 o 11 dígitos, pero se ingresaron %.', CHAR_LENGTH(v_ruc_text); "
                            +
                            "        END IF; " +
                            "        NEW.RUC := v_ruc_text; " +
                            "    ELSE " +
