...
public class CargaDb5 {
    public static void ejecutar(Connection conn) {
        try (Statement st = conn.createStatement()) {
            ...
            st.executeUpdate(
                "CREATE OR REPLACE PROCEDURE SP_ELIMINAR_TEL_CLIENTE(IN p_id_telcli INT, IN p_id_cli INT) LANGUAGE sql AS $$ DELETE FROM Telefono_Cli WHERE ID_TELCLI = p_id_telcli AND ID_CLI = p_id_cli; $$;"
            );
            // ... SERVICIO_PRODUCTO
            st.executeUpdate(
