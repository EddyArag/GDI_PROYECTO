...
public class CargaDb3 {
    public static void ejecutar(Connection conn) {
        try (Statement st = conn.createStatement()) {
            ...
            st.executeUpdate("CREATE INDEX IF NOT EXISTS idx_cliente_nombre_completo ON Cliente (P_NOMB, APE_P, APE_M);");
            st.executeUpdate("CREATE INDEX IF NOT EXISTS idx_sp_descripcion_trgm ON Servicio_Producto USING GIN (DESCP gin_trgm_ops);");
            ...
            st.executeUpdate("CREATE INDEX IF NOT EXISTS idx_cotdet_servicio ON Cotizacion_Detalle (ID_SERV);");
        } catch (Exception ex) {
